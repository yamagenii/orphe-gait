...
if exist ('OCTAVE_VERSION', 'builtin') 
	...
	%time = xIMUdata(:,1)*samplePeriod; %time sec
	%gyrX = xIMUdata(:,2);
	%gyrY = xIMUdata(:,3);
	%gyrZ = xIMUdata(:,4);
	%accX = xIMUdata(:,5);
...
%orpheTime = orpheTime(indexSel);
orpheQW = orpheQW(indexSel, :);
orpheQX = orpheQX(indexSel, :);
orpheQY = orpheQY(indexSel, :);
orpheQZ = orpheQZ(indexSel, :);
...
quat = [orpheQW orpheQX orpheQY orpheQZ]
